...
def get_roi_at_each_frame(bp_data, rois, check_inroi):
    ...
    def sort_roi_points(roi, circle_1, center_x, center_y):
        return np.sort(circle_1), np.sort(circle_1)
    ...
    Need to implement a way to directly pull x and y from the load and analyze file to limit amount of steps and just unnecessary shizzle
    
    '''
    centers = []
    for points in rois.values(): # need to change so recognizes boundaries for points inside of circle instead of hard boundaries
    ...
    if not check_inroi: 
        ...
        for i, roi in enumerate(roi_at_each_frame):
            ...
            if not (x-center_x)^2 + (y - center_y)^2 < radius^2:
